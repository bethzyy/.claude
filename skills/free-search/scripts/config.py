...
def get_zhipu_api_key() -> str:
    ...
    # 方式3: LLM_Configs目录（第二个API key）
    llm_config_path = Path('C:/D/CAIE_tool/LLM_Configs/GLM/apikeyValue2.txt')
    if llm_config_path.exists():
        try:
            with open(llm_config_path, 'r', encoding='utf-8') as f:
                key = f.read().strip()
...
def get_serper_api_key() -> str:
    ...
    安全获取Serper API Key
    优先级：
    1. 环境变量 SERPER_API_KEY
    2. 项目目录下的 .env 文件
    3. C:\D\CAIE_tool\LLM_Configs\serper\apikey.txt（自动检测）
    """
    # 方式1: 环境变量（推荐）
    ...
    # 方式3: LLM_Configs目录（项目默认配置位置）
    llm_config_path = Path('C:/D/CAIE_tool/LLM_Configs/serper/apikey.txt')
    if llm_config_path.exists():
        try:
            with open(llm_config_path, 'r', encoding='utf-8') as f:
                key = f.read().strip()
