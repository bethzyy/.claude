...
搜索源:
  auto        自动选择（默认，依次尝试Tavily → Serper → DuckDuckGo → AI知识库）
  tavily      使用Tavily API（需要配置TAVILY_API_KEY）
  serper      使用Serper.dev（Google搜索结果API，需要配置SERPER_API_KEY）
  ...
  ai_knowledge 使用AI知识库（需要配置ZHIPU_API_KEY，仅预训练知识）
    ...
    parser.add_argument('--format', choices=['text', 'json', 'markdown'], default='text',
                        help='输出格式（默认: text）')
    parser.add_argument('--source', choices=['auto', 'tavily', 'serper', 'duckduckgo', 'ai_knowledge'],
                        default='auto', help='指定搜索源（默认: auto，可选: auto, tavily, serper, duckduckgo, ai_knowledge）')
