---
name: free-search
description: FREE multi-level fallback web search skill with automatic provider switching. Tries Tavily → Serper → DuckDuckGo → AI knowledge base. Zero cost, always available. Use when user asks to "search web", "find information", "look up online", "免费搜索", "搜索一下", or any web search request.
version: 3.3.0
entry_point: main.py
author: Claude Code
tags: [search, web, fallback, free, tavily, serper, duckduckgo]
---
...
- **🔍 New Serper.dev Provider**: Added Google search results API as fallback option
  - **Position**: Between Tavily and DuckDuckGo (Priority 3)
  - **Quality**: Higher quality results than DuckDuckGo
  - **API Key**: Auto-detected from `C:\D\CAIE_tool\LLM_Configs\serper\apikey.txt`
  - **Chinese Support**: Configured with `hl: zh-cn`, `gl: cn` for better Chinese results
...
1. **Tavily API** (Priority 1 - **First Choice**)
...
2. **Serper.dev** (Priority 2)
   ...
   - Auto-detects API key from `C:\D\CAIE_tool\LLM_Configs\serper\apikey.txt`
   - High quality, real-time Google results
   - Supports Chinese search with `hl: zh-cn`, `gl: cn`
...
**Auto-detected**: The skill automatically reads from `C:\D\CAIE_tool\LLM_Configs\serper\apikey.txt` if it exists.

You can also set `SERPER_API_KEY` environment variable:
...
Get API key at: https://serper.dev (Free tier: 2500 searches/month)
...
search_engine.py (Core engine)
    ...
    └── Providers (Priority order)
        ...
        ├── SerperProvider (Google search results API)
...
6. **Simplified architecture**: Removed ZhipuMCP for cleaner codebase
...
- **v3.3.0** (2026-03-19): Remove ZhipuMCP provider
  - Removed ZhipuMCPProvider to simplify architecture
  - Fallback order updated: Tavily → Serper → DuckDuckGo → AI Knowledge
  - Reduced from 5-level to 4-level fallback mechanism
  - Cleaner codebase with fewer dependencies

- **v3.1.0** (2026-03-19): Add Serper.dev provider
  - Added SerperProvider as Priority 3 (between Tavily and DuckDuckGo)
  - Google search results API with Chinese support (hl: zh-cn, gl: cn)
  - Auto-detects API key from `C:\D\CAIE_tool\LLM_Configs\serper\apikey.txt`
