---
name: web-search
description: Multi-level fallback web search skill with automatic provider switching. Tries Tavily → DuckDuckGo → AI knowledge base. Use when user asks to "search web", "find information", "look up online", "find latest info about", or any web search request.
version: 1.0.0
entry_point: main.py
author: Claude Code
tags: [search, web, fallback, tavily, duckduckgo]
---
...
1. **MCP WebSearch** (Priority 1 - **First Choice**)
   - Uses ZhipuAI GLM Coding Plan quota
   - Built-in MCP tools (WebSearch, webSearchPrime, webReader)
   - No API key required (uses Claude Code environment)
...
3. **DuckDuckGo** (Priority 3)
   ...
   - Reliable fallback when Tavily quota is exhausted
...
search_engine.py (Core engine)
    ...
    └── Providers (Priority order)
        ├── TavilyProvider (API-based, high quality)
...
1. **Reliability**: 3 independent search sources, ensures at least one works
