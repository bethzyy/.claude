...
```bash
python C:/D/CAIE_tool/MyAIProduct/skills/toutiao-cnt/toutiao_content.py create <topic> [options]
```
...
```bash
python skills/toutiao-cnt/toutiao_content.py create "元宵节风俗"
```

### 2. Create with Specific Output Directory
```bash
python skills/toutiao-cnt/toutiao_content.py create "RPA自动化" --output-dir "C:/D/CAIE_tool/MyAIProduct/post/article/他山之石"
```

### 3. Create with Custom Style
```bash
python skills/toutiao-cnt/toutiao_content.py create "人工智能发展" --style academic
```
...
1. **Specify topic:**
   ```bash
   python skills/toutiao-cnt/toutiao_content.py create "元宵节各地风俗"
   ```
...
```bash
python C:/D/CAIE_tool/MyAIProduct/skills/toutiao-cnt/integrate_content.py <html_file> <new_content> [options]
```
...
```bash
python skills/toutiao-cnt/integrate_content.py article.html "## 新章节
...
```bash
python skills/toutiao-cnt/integrate_content.py article.html content.md --position after --after "工具对比"
```

### 3. Insert Before Specific Section
```bash
python skills/toutiao-cnt/integrate_content.py article.html new_content.html --position before --before "总结"
```

### 4. Smart Merge (Auto-skip Duplicates)
```bash
python skills/toutiao-cnt/integrate_content.py article.html updates.md --merge
```

### 5. List Existing Sections
```bash
python skills/toutiao-cnt/integrate_content.py article.html --list-sections
```
...
**Script location**: `C:\D\CAIE_tool\MyAIProduct\skills\toutiao-cnt\integrate_content.py`
